...
data = read.csv('~/Dropbox/JOB/DS/employee_retention_data.csv')
...
unique_dates = seq(as.Date("2011/01/24"), as.Date("2015/12/13"), by = "day") # cre􀀁ate list of unique dates for the table
...
#NowI left outer join with data_headcount.
...
#NowI need the sum by company_id. Data set is already ordered by date,
...
#Create binary class
data = subset(data, data$join_date < as.Date("2015/12/13") - (365 + 31)) # only keep people who had 􀁈􀁑􀁒􀁘􀁊􀁋 time to age
...
tree #we are not too interested in predictive powe􀁕, we are mainly using the teeas a descriptive stat tool.
